 ...
 *
 * Nicht frei wählbar. Bei 0,30 lag die gemalte Figur ΔE 14 von ihrer
 * Spielerfarbe entfernt, und Grün und Schwarz rückten auf ΔE 22,8 zusammen –
 * unter die Grenze der Palette. Gemessen über alle Untergründe des Spiels
 * ist 0,125 der größte Wert, bei dem beides hält – über dem Wasserblau
 * der Flusskarten wird es sonst wieder eng (Grün/Schwarz auf ΔE 24,6).
 */
const SPREIZUNG = 0.125;

/** Die Tafel in der Mitte einer Facettengruppe – die flache Fläche obenauf. */
const TAFEL = 0.10;

/** Benachbarte Facetten sind nie exakt gleich geneigt. */
const WECHSEL = 0.045;
...
export const KOERPER = [
  { name: 'Kopf', anteil: 0.13, cx: 50, cy: 23, ri: 6.5, ro: 30, n: 8, dreh: 0.20,
    d: 'M41.5 37.5 A17 17 0 1 1 58.5 37.5 Q50 42 41.5 37.5 Z' },
  { name: 'Brust', anteil: 0.11, cx: 50, cy: 44, ri: 4.5, ro: 26, n: 7, dreh: 0.45,
    d: 'M41.5 37.5 Q50 42 58.5 37.5 Q62 43.5 65.5 50 Q50 53.5 34.5 50 Q38 43.5 41.5 37.5 Z' },
  { name: 'Bauch', anteil: 0.16, cx: 50, cy: 58, ri: 5.5, ro: 28, n: 8, dreh: 0.10,
    d: 'M34.5 50 Q50 53.5 65.5 50 Q68 58 68.4 66.2 Q50 71.5 31.6 66.2 Q32 58 34.5 50 Z' },
  { name: 'Oberarm links', anteil: 0.10, cx: 30, cy: 52, ri: 4, ro: 26, n: 6, dreh: 0.55,
    d: 'M42.3 38.1 Q32 48 31.6 66.2 L4 52 L-1 -1 Z' },
  { name: 'Unterarm links', anteil: 0.06, cx: 17, cy: 66, ri: 3.5, ro: 22, n: 6, dreh: 0.15,
    d: 'M31.6 66.2 L4 52 L-1 82 Z' },
  { name: 'Oberarm rechts', anteil: 0.10, cx: 70, cy: 52, ri: 4, ro: 26, n: 6, dreh: 0.30,
    d: 'M57.7 38.1 Q68 48 68.4 66.2 L96 52 L101 -1 Z' },
  { name: 'Unterarm rechts', anteil: 0.06, cx: 83, cy: 66, ri: 3.5, ro: 22, n: 6, dreh: 0.65,
    d: 'M68.4 66.2 L96 52 L101 82 Z' },
  { name: 'Bein links', anteil: 0.14, cx: 34, cy: 84, ri: 4.5, ro: 26, n: 7, dreh: 0.25,
    d: 'M50 69 L31 66.6 L-1 66.6 L-1 101 L50 101 Z' },
  { name: 'Bein rechts', anteil: 0.14, cx: 66, cy: 84, ri: 4.5, ro: 26, n: 7, dreh: 0.50,
...
export function facetten(k) {
  ...
  const tafel = [];
  for (let i = 0; i < k.n; i++) tafel.push(p(k.ri, k.dreh + (i / k.n) * TAU));
  aus.push({ d: pfad(tafel), winkel: null, ton: TAFEL });

  for (let i = 0; i < k.n; i++) {
    const a0 = k.dreh + (i / k.n) * TAU;
    const a1 = k.dreh + ((i + 1) / k.n) * TAU;
    const mitte = (a0 + a1) / 2;
    // Lambert: die Facette ist am hellsten, wenn ihre Normale zum Licht
    // zeigt. Dazu ein kleiner Wechsel, weil benachbarte Facetten eines
    // Schliffs nie exakt gleich geneigt sind.
    const ton = SPREIZUNG * Math.cos(mitte - LICHT) + (i % 2 ? -WECHSEL : WECHSEL);
    aus.push({ d: pfad([p(k.ri, a0), p(k.ro, a0), p(k.ro, a1), p(k.ri, a1)]), winkel: mitte, ton });
  }
  return aus;
}
...
export function facettenTon(color, ton) {
  ...
  return ton < 0 ? mix(grund, color, 0.35) : grund;
}

/** Alle Facettenfarben der ganzen Figur, in Zeichenreihenfolge. */
export function glasToene(color) {
  const aus = [];
  for (const k of KOERPER) for (const f of facetten(k)) aus.push(facettenTon(color, f.ton));
  return aus;
}
...
export function glasMittel(color) {
  ...
  for (const k of KOERPER) {
    ...
    for (const f of fs) {
      const c = rgb(facettenTon(color, f.ton));
